...
namespace BanTien.Controllers
{
    public class GioHangController : Controller
    {
        ...
        [Authorize]
        [HttpPost]
        public ActionResult AddItem(string MaKH, string MaTien, string quantity, string controller, string action)
        {
            int kh = Int32.Parse(MaKH);
            int sach = Int32.Parse(MaTien);
        ...
        public GioHang existInCart(int kh, int sach)
        {
            return (from u in db.GioHangs
                    where u.MaKH == kh && u.MaTien == sach && u.MaDH == null
        ...
        public ActionResult ModifyQuantity(int gh, int kh, int quantity, double dongia)
        {
            ...
            var sp = db.SanPhams.Find(cart.MaTien);
